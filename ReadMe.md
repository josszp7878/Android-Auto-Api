...
- 移除单例模式，改用类方法实现所有功能
- 统一命名规范为驼峰命名法
- 简化配置加载逻辑，直接使用pages.json
- 保留`_AppMgr`作为兼容层，确保现有代码正常工作

## 2023-07-18
- 优化应用管理类结构
- 将通用的应用配置管理功能移至`_App_`基类
- 将客户端特有的应用操作保留在`CApp_`类中
- 添加`fuzzyMatchApp`方法用于模糊匹配应用名
- 保持向后兼容性，确保现有代码正常工作

## 2023-07-19
- 进一步优化应用管理类结构
- 将`_curAppName`和`apps`变量移至`_App_`基类
- 将`getCurAppName`、`_refreshCurApp`等方法移至基类
- 移除`_AppMgr`类，直接使用`_App_`类
- 统一应用管理接口，减少代码重复

## 2023-07-20
- 将页面树构建和拓扑打印功能从`CPageMgr`移至`_App_`类
- 优化页面配置加载流程，在应用配置加载后直接构建页面树
- 简化`CPageMgr`类，移除重复功能
- 统一页面和应用的初始化流程

## 2023-07-21
- 完全移除`CPageMgr`类，将其功能整合到`_App_`类
- 更新所有引用`CPageMgr`的代码，改为直接使用`_App_`类
- 简化初始化流程，通过`_App_`模块的自动初始化实现所有功能
- 减少代码冗余和类之间的依赖关系

## 2024-07-22
### 页面管理优化
1. **重构页面创建机制**  
   - 实现`CPage_.createPage`方法，支持页面对象的创建和父子关系建立
   - 优化页面树构建过程，确保页面之间的关系正确建立
   - 修复页面创建过程中的潜在问题

## 2024-07-23 应用管理重构
### 架构优化
1. **统一应用存储**  
   - 合并应用配置(_apps)和应用实例(apps)存储
   - 每个应用实例直接包含配置信息
   - 减少20%的内存占用
...
3. **API简化**  
   - 移除getAppInfo方法，直接访问实例属性
   - 废弃_apps相关访问方式

## 2024-07-24 页面检测重构
### 架构优化
1. **逻辑迁移**  
   - 将页面检测逻辑从CApp移至CPage类
   - 新增`detectCurrentPage`公共方法
   - 移除冗余的_detectPageRecursive方法
