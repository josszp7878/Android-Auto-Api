...
## 2025-01-01 功能更新
### 优化前台应用获取方法
- **性能优化**: 简化`getCurrentApp()`方法，移除无效方案，只保留经测试最可靠的UsageStatsManager方案
- **智能缓存**: 添加前台应用缓存机制，解决短时间内无应用切换时返回空的问题
- **优化内容**:
  - 移除无障碍服务方案（方案1）和ActivityManager方案（方案2）
  - 只保留UsageStatsManager方案（方案3），经测试在华为等设备上最可靠
  - 新增缓存机制：当最近时间段无应用切换记录时，返回缓存的前台应用信息
  - 缓存有效期：5分钟，确保信息时效性
- **影响**: 提高前台应用识别的准确性、稳定性和用户体验

### 修复文件路径分隔符问题
- **重要修复**: 解决了Windows服务器环境下，文件下载到Android端时路径分隔符错误的问题
- **修复内容**:
  - **服务端** (SRoutes.py): `get_timestamps()`函数统一使用正斜杠`/`作为路径分隔符
  - **Android端** (ScriptEngine.java): `downloadFile()`方法对路径进行标准化处理
- **问题影响**: 修复前可能导致`ModuleNotFoundError: No module named 'CMain'`等Python模块导入失败

### 新增客户端文件操作指令
- **新增功能**: 添加客户端文件查看和读取功能，用于调试和诊断
- **客户端指令**:
  - `#列出文件|lcwj` - 列出客户端scripts目录下的所有文件
  - `#读取文件|dcwj [文件名]` - 读取客户端指定文件的内容
- **Android端新方法** (PythonServices.kt):
  - `readFileContent(fileName)` - 读取指定文件内容
  - `listScriptsFiles()` - 列出scripts目录下所有文件
- **应用场景**: 当Python模块导入失败时，可以通过这些指令检查文件是否正确下载和保存

---

### 2024-06-14 - 设备表格状态排序优化

- #前端设备表格支持自定义状态排序：
  - 设备状态显示顺序调整为：离线 > 登出 > 在线 > 登录
  - 通过自定义排序函数，保证离线设备始终排在最前，登录设备排在最后
  - 代码位置：server/static/js/sheet.js 的 getDeviceColumns()
