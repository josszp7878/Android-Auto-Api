...
## 2024-05-26 更新
### UI优化与触摸监控改进
- 将日志切换按钮移至底部输入栏，提高操作便捷性
- 修复底部输入栏位置，确保正确贴合屏幕底部
- 优化触摸监控视图创建逻辑，按需创建而非默认启用
- 解决透明背景挡住其他应用点击的问题
- 改进UI组件布局，提高整体用户体验
...
## 2023-12-20
### 功能优化
- 完善设备排序逻辑：在线设备优先，按最后活跃时间倒序
- 修复设备连接/断开时最后活跃时间更新问题
- 增强设备状态变化的响应式更新
...
- 修改应用名称为"点点"
- 修复应用桌面图标显示不完整问题
- 为应用图标添加白色背景，确保图标在各种启动器上正确显示
- 优化图标配置，解决图标被裁剪的问题

## 2024-05-29
### 悬浮按钮与输入框交互优化
- 实现悬浮按钮与输入框的联动效果
- 当输入框显示时，悬浮按钮自动停靠在输入界面左上方
- 停靠状态下禁止拖动悬浮按钮，保持界面整洁
- 当输入框隐藏时，悬浮按钮恢复原位置并可自由拖动
- 优化用户体验，使界面元素之间的关系更加直观

## 2024-05-31
### 代码优化与稳定性提升
- 统一工具栏可见性控制逻辑，提高代码一致性
- 修复悬浮按钮重复创建问题，优化内存使用
- 完善日志区域切换逻辑，确保UI状态一致性
- 优化触摸监控视图的创建逻辑，提高应用稳定性
- 修复空指针异常问题，增强应用健壮性
...
- 修复悬浮按钮点击不显示输入栏的问题
- 修复日志切换按钮不显示的问题
- 优化命令历史显示顺序，最近的命令显示在底部
- 改进命令历史浏览逻辑，使其更符合用户习惯
- 增强UI元素之间的交互一致性
...
## 2024-06-03
### 命令历史管理优化
- 重构命令历史存储逻辑，采用标准的历史管理方式
- 最新命令添加到列表末尾，浏览时从新到旧遍历
- 优化重复命令处理，自动移除旧记录保留最新执行
- 简化历史浏览逻辑，提高代码可维护性
- 修复命令历史对话框崩溃问题，增强稳定性
...
### 界面布局优化
- 将日志界面与命令输入栏合并为一体化设计
- 日志区域现在位于输入栏上方，占据屏幕大部分空间
- 日志界面下边对齐输入框界面的上边，上边界对齐屏幕的上边
- 保持原有的命令输入功能和按钮布局不变
- 优化整体视觉效果，提高界面空间利用率

## 2024-06-06
### 界面布局进一步优化
- 将工具栏按钮（开始/停止、同步、设置）整合到日志区域内部底部
- 保留原有按钮图标样式，使用系统内置图标确保一致性
- 形成更紧凑的两段式布局：日志区域（含工具栏）和命令输入区
- 优化按钮排列和间距，提高视觉整体性
- 完善一体化设计，使界面更加简洁直观

## 2023-07-15
- 优化了触摸监控视图的实现方式
- 将触摸监控视图集成到布局文件中，而不是动态创建
- 确保触摸监控视图位于底部输入栏和切换按钮下方
- 改进了触摸事件的处理和显示
...
## 2023-07-19
- 优化了命令注册机制，支持简化的命令格式
- 添加了 `#命令名|别名` 格式，自动转换为正则表达式
- 支持在完整正则表达式中嵌入简化格式
- 提高了命令定义的可读性和可维护性

## 2023-07-20
- 统一修改了所有命令注册装饰器格式
- 将 `@regCmd(r"(?:命令名|别名)...")` 修改为 `@regCmd(r"#命令名|别名 ...")`
- 优化了命令定义的可读性和一致性
- 简化了命令注册过程，提高了代码可维护性

## 2023-07-21
- 完成了所有命令注册装饰器的格式统一
- 修改了 SCmds.py 中剩余的命令注册格式
- 确保所有命令使用 `#命令名|别名` 格式
- 提高了整个代码库的一致性和可维护性
...
- 修正了 SCmds.py 中的冒号错误
- 完成了所有命令的格式统一
- 确保所有命令注册使用 r 前缀的原始字符串

## 2023-07-23
- 增强命令注册机制，自动添加方法名和方法名缩写到命令别名
- 方法名缩写规则：取方法名第一个字母和所有大写字母组合
- 例如：`showClick` 的缩写为 `sC`，`back` 的缩写为 `b`
- 用户不再需要手动添加这些匹配项，系统自动处理
- 简化了命令定义，同时保持了命令的可发现性

## 2023-07-24
- 优化命令注册机制，统一处理函数名和缩写
- 无论装饰器是否有参数，都自动添加函数名和缩写到命令别名
- 简化了命令定义，用户只需关注主要命令名和别名
- 提高了命令的可发现性，支持通过函数名和缩写直接调用命令

## 2023-07-25
- 修复了命令注册机制中的正则表达式匹配问题
- 改进了带参数命令的处理逻辑，确保正确匹配命令名部分
- 添加了命令注册日志，便于调试和排查问题
- 解决了某些命令无法正确注册和匹配的问题
...
- 修复了命令注册中的空格处理问题
- 改进了命令名和参数之间的空格匹配逻辑
- 确保命令可以正确识别，无论命令名和参数之间有多少空格
- 优化了正则表达式生成，提高了命令匹配的灵活性

## 2023-07-27
- 增强命令注册机制，支持参数之间的空格匹配
- 自动在参数捕获组之间添加\s+模式，确保参数之间必须有空格
- 优化了命令解析逻辑，提高了命令格式的一致性
- 改进了命令参数处理，使命令更加灵活和易用

## 2023-07-28
- 修复了命令注册机制中的变量引用错误
- 添加了对 pattern 为 None 的处理，使用函数名作为默认命令名
- 使用 nonlocal 声明确保正确引用外部作用域的变量
- 增强了命令注册的健壮性和错误处理能力

## 2023-07-29
- 修复了参数间空格匹配问题，确保只在参数之间添加\s+
- 优化了命令匹配逻辑，支持大小写不敏感匹配
- 改进了命令缩写生成规则，保持原始大小写格式
- 增强了命令系统的容错能力和匹配准确性

## 2023-07-30
- 精确修复参数间空格处理问题
- 确保\s+只添加在参数之间，不在参数之后
- 改进了参数位置检测算法，支持处理可选参数
- 增强了正则表达式生成的准确性
