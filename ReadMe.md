...
## 桌面作为应用处理
- 将桌面(Top)作为特殊应用加入到apps集合中
- 统一应用跳转和页面查找逻辑
- 优化应用切换和页面导航流程
- 增强桌面状态检测和处理

## 技术细节
- 新增`initTopApp()`方法创建桌面应用实例
- 修改`gotoApp()`支持桌面应用跳转
- 优化`_refreshCurApp()`检测桌面状态
- 重写`loadConfig()`确保Top应用初始化

## 2024-06-02 更新

### 应用检测机制优化
- 将应用配置中的alerts字段改名为checkers
- 优化页面检测逻辑，使用更语义化的命名
- 相关方法名从checkAlerts改为checkCheckers
- 保持功能不变，仅优化代码可读性
