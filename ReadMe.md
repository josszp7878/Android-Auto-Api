...
### 代码重构与功能增强
- 将 `CmdMgr` 重命名为 `CmdMgr`，提高命名一致性
- 新增服务端命令注册系统 `SCmds.py`
  - 类似客户端的 `CCmds.py`，实现服务端指令的统一注册
  - 使用装饰器方式注册命令，简化代码
  - 支持命令描述和帮助信息
- 添加了 `@date` 命令用于测试服务端命令系统
  - 用法: `@date [format]`
  - 显示当前日期和时间，支持自定义格式
...
### 代码重构与优化
- 移除了 `SCommand` 类，使用 `CmdMgr` 统一管理服务端和客户端命令
- 所有服务端命令现在通过 `SCmds.py` 使用 `@regCmd` 装饰器注册
- 命令注册格式更新：`@regCmd(命令名, 参数匹配模式, 命令描述)`
- 参数匹配使用正则表达式，支持命名捕获组
- 优化了命令处理流程，提高了代码可维护性

## 2023-11-15
- 修复了命令管理器中IDE无法识别实例属性的问题
- 在类定义中声明了cmdRegistry和nameRegistry属性，同时保持单例模式的正确性
- 移除了未使用的导入模块
- 修复了代码格式问题，确保符合PEP8规范

### 命令超时机制

- 添加了命令执行超时机制，确保即使网络出现问题，也能正确处理命令执行结果
- 服务端发送命令时会生成唯一的命令ID，并设置超时定时器
- 如果在指定时间内未收到客户端响应，将自动触发超时处理
- 客户端响应时会返回命令ID，服务端据此取消对应的超时定时器
- 默认超时时间为10秒，可根据需要调整

## 2024-06-20 更新日志
# 修复 logger.py 中的 linter 错误
- 修复了代码格式问题，符合PEP 8规范
- 优化了Log类的单例模式实现，确保所有地方使用同一个实例
- 修复了注释格式和空行问题
- 修复了行长度超过79字符的问题
- 修复了缩进问题
