...
class SAppMgr:
    ...
    def _load_config(self):
        ...
        try:
            ...
            _Log.i(f"已加载{len(self._apps)}个应用配置")
    ...
    def getRatio(self, app_name: str) -> float:
        ...
        try:
            ...
            _Log.w(f"应用[{app_name}]未配置，使用默认比例0.01")
            return 0.01
        except Exception as e:
            _Log.ex(e, "获取应用换算比例失败")
            return 0.01
    ...
    def getApp(self, app_name: str, showLog: bool=True) -> str:
        ...
        try:
            ...
            if matches:
                ...
                _Log.i(f"应用[{app_name}]模糊匹配到[{matches[0][0]}]")
                return matches[0][0]
        except Exception as _:
            pass
        finally:
            if showLog:
                _Log.i(f"应用[{app_name}]匹配失败")
        return None
