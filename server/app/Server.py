...
def handle_connect(auth=None):
    ...
    try:
        ...
        _Log.i(f'收到连接请求: {device_id} {client_type}')
...
def handle_stop_task(data):
    ...
    try:
        ...
        app_name = data.get('app_name')
        task_name = data.get('task_name')
        _Log.i(f'收到任务停止消息: {device_id}/{app_name}/{task_name}')
        device = deviceMgr.get_device(device_id)
        if not device:
            _Log.e(f'设备不存在: {device_id}')
            return
...
def handle_cancel_task(data):
    ...
    try:
        ...
        app_name = data.get('app_name')
        task_name = data.get('task_name')
        _Log.i(f'收到任务取消消息: {device_id}/{app_name}/{task_name}')
        device = deviceMgr.get_device(device_id)
        if not device:
            _Log.e(f'设备不存在: {device_id}')
            return
...
def handle_B2S_FilterLogs(data):
    ...
    try:
        ...
        # 应用过滤器
        _Log().Filter(filter_str)
        
        # 刷新日志显示
        _Log.show(page=page)
    except Exception as e:
        _Log.ex(e, '处理日志过滤请求失败')
