...
class CFileServer:
    ...
    def download(self, filename, onComplete=None):
        ...
        try:
            ...
            scriptFile = os.path.join(_Log.scriptDir(), filename)
            _Log.i(f"保存文件到: {scriptFile}")
            ...
            _Log.i(f"下载完成: {scriptFile} (大小: {os.path.getsize(scriptFile)} bytes)")
            if onComplete:
                onComplete(True)
            return True
        except Exception as e:
            _Log.ex(e, f"下载失败: {filename}")
            if onComplete:
                onComplete(False)
            return False

    def currentVersions(self):
        version_file = os.path.join(_Log.scriptDir(), "version.txt")
