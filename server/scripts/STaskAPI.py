...
def register_socket_events(socketio):
    ...
    @socketio.on('get_daily_tasks')
    def handle_get_daily_tasks(data):
        """处理获取当天任务的请求"""
        log = _G._G_.Log()
        date_str = data.get('date', datetime.now().strftime('%Y-%m-%d'))
        log.i('TaskAPI', f'Socket请求{date_str}的任务数据')
        ...
        try:
            ...
            log.e('TaskAPI', f'Socket获取日期任务数据失败: {str(e)}')
            emit('error', {'message': f'获取任务数据失败: {str(e)}'})
    ...
    def handle_execute_task(data):
        ...
        log.i('TaskAPI', f'Socket请求执行任务: {task_id}')
        
        # 解析任务ID
        parts = task_id.split('_')
        if len(parts) < 2:
            emit('error', {'message': f'任务ID格式错误: {task_id}'})
            return
        ...
        if not device or not hasattr(device, 'taskMgr'):
            ...
            emit('error', {'message': f'设备{deviceId}不存在或没有任务管理器'})
            return
        ...
        try:
            ...
            if not task:
                emit('error', {'message': f'无法创建任务: {taskName}'})
                return
            
            success = device.taskMgr.startTask(task)
            if success:
                log.i('TaskAPI', f'任务{task_id}开始执行')
                emit('success', {'message': f'任务{task_id}已开始执行'})
            else:
                emit('error', {'message': '启动任务失败'})
        except Exception as e:
            log.e('TaskAPI', f'执行任务{task_id}失败: {str(e)}')
            emit('error', {'message': f'执行任务失败: {str(e)}'})
    ...
    def handle_pause_task(data):
        ...
        log = _G._G_.Log()
        task_id = data.get('taskId')
        log.i('TaskAPI', f'Socket请求暂停任务: {task_id}')
        ...
        try:
            ...
            if success:
                ...
                emit('success', {'message': f'任务{task_id}已暂停'})
            else:
                emit('error', {'message': '暂停任务失败'})
        except Exception as e:
            log.e('TaskAPI', f'暂停任务{task_id}失败: {str(e)}')
            emit('error', {'message': f'暂停任务失败: {str(e)}'})
    ...
    def handle_cancel_task(data):
        ...
        log = _G._G_.Log()
        task_id = data.get('taskId')
        log.i('TaskAPI', f'Socket请求取消任务: {task_id}')
        ...
        try:
            ...
            if success:
                ...
                emit('success', {'message': f'任务{task_id}已取消'})
            else:
                emit('error', {'message': '取消任务失败'})
        except Exception as e:
            log.e('TaskAPI', f'取消任务{task_id}失败: {str(e)}')
            emit('error', {'message': f'取消任务失败: {str(e)}'})
